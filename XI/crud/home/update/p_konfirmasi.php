...
if (
    ...
    foreach (@$_SESSION['list'] as $key_produk => $val_produk) {
        ...
        $dt_produk->bind_param('ssssss', $_SESSION["id_user"], $val_produk["id_produk"], $_SESSION["nama"], $val_produk["nama_produk"], $val_produk["jumlah"], date("Y-m-d"));
        $dt_produk->execute();
        $dt_produk->close();
        ...
        $dt_produk->bind_param('ss', $val_produk["jumlah"], $val_produk["id_produk"]);
        $dt_produk->execute();
    }
    unset($_SESSION['list']);
    $msg = "berhasil";
